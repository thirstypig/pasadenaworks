...
export const copy: Partial<Record<CitySlug, CityCopy>> = {
  ...
  'san-marino': {
    ...
    body: [
      ...
      'With only 37 dentists, optometrists, and primary-care physicians registered in the entire city, a San Marino practice has few local competitors to win patients from, so its growth lies with the patients it already has and with patients who find it from the surrounding cities. Get more patients addresses both. It sends recall messages to patients who are overdue for a visit, and it writes the Google Business Profile and the website in English and Chinese, so that a patient comparing San Marino practices with those in Pasadena or San Gabriel finds the same details in either language.',
    ],
