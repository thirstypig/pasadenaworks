...
- One blog pillar per live service — `websites`/`digitize`/`consulting`/`transition`, replacing a taxonomy that still named `search` and `ads` after those services were retired, and that funnelled 48 of 68 posts (71%) at one service while two services had none (2026-09-23, #96)
- All 61 unpublished posts are triaged KEEP/ADAPT/REPLACE in `CONTENT-PLAN.md`, and the four due before the publishing front are re-aimed at practices in all four languages (2026-09-23, #97). Two of them argued from the wrong Census variable — a city's Hispanic or Asian SHARE rather than **language spoken at home**, table C16001. Ethnicity is not language; use C16001, which the city pages already cite. Watch for the same substitution in the remaining 57
